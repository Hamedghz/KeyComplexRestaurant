// Lightweight production-ready carousel
// Features: autoplay, pause on hover, touch swipe, keyboard, lazy load, prefetch next, GPU transform
export default class Carousel{
  ...
  start(){
    if(this.autoplay && !this.timer){
      this.timer = setInterval(()=>{ if(!this.isPaused) this.goTo(this.index+1) }, this.autoplay)
    }
  }
  ...
  prefetchNext(){
    const nextSlide = this.slides[(this.index+1)%this.count]
    const img = nextSlide.querySelector('img[data-src]')
    if(img && !img.src){ const p=new Image(); p.src = img.dataset.src }
  }
}
