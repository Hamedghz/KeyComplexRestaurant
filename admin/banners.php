...
$config = adminModuleDefinition('banners');
if (!$config) {
...
try {
    ...
    safeAdminLog('Admin module bootstrap failed (banners): ' . $e->getMessage());
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$error) {
    try {
        ...
        safeAdminLog('Admin module save failed (banners): ' . $e->getMessage());
...
try {
    ...
    safeAdminLog('Admin module rows failed (banners): ' . $e->getMessage());
}
