...
if (!function_exists('adminRenderSafeError')) {
    function adminRenderSafeError(string $pageTitle, string $logMessage, string $displayMessage = 'درخواست قابل پردازش نیست. جزئیات خطا در لاگ سیستم ثبت شد.'): void {
        safeAdminLog($logMessage);
        http_response_code(500);
        if (defined('CSRF_TOKEN_NAME')) {
            include __DIR__ . '/../includes/header.php';
            echo '<div class="alert" style="background:#f8d7da;color:#721c24">' . h($displayMessage) . '</div>';
            include __DIR__ . '/../includes/footer.php';
            return;
        }
        echo h($displayMessage);
    }
}
...
function ensureAdminSchema(): array {
    ...
    if (adminTableExists('employee_evaluations')) {
        ...
        if (!adminIndexExists('employee_evaluations', 'idx_eval_evaluator')) {
            $run('ALTER TABLE `employee_evaluations` ADD INDEX `idx_eval_evaluator` (`evaluator_id`)', 'ایندکس ارزیاب employee_evaluations');
        }
        if (!adminIndexExists('employee_evaluations', 'idx_eval_employee_month')) {
            $run('ALTER TABLE `employee_evaluations` ADD INDEX `idx_eval_employee_month` (`employee_id`, `period_month`)', 'ایندکس employee_evaluations');
        }
        if (!adminIndexExists('employee_evaluations', 'idx_eval_period_month')) {
            $run('ALTER TABLE `employee_evaluations` ADD INDEX `idx_eval_period_month` (`period_month`)', 'ایندکس ماه employee_evaluations');
        }
        if (!adminIndexExists('employee_evaluations', 'uniq_eval_once')) {
            $run('ALTER TABLE `employee_evaluations` ADD UNIQUE KEY `uniq_eval_once` (`evaluator_id`, `employee_id`, `period_month`)', 'ایندکس یکتای employee_evaluations');
        }
    }
...
function schemaColumns(string $table): array {
    ...
    try {
        ...
        safeAdminLog('Schema columns lookup failed for table=' . $table . ' error=' . $e->getMessage());
        return [];
    }
}
...
function adminCanonicalColumnDefinitions(string $table): array {
    $statement = adminCanonicalCreateStatement($table);
    ...
    foreach (preg_split('/\R/', $statement) ?: [] as $line) {
        ...
        $column = $matches[1];
        $definition = rtrim(trim($matches[2]), ',');
        $definition = adminSharedHostingCompatibleCreate($definition);
        if (stripos($definition, 'AUTO_INCREMENT') === false && stripos($definition, 'NOT NULL') !== false && stripos($definition, 'DEFAULT') === false) {
...
function adminModuleConfiguredColumnNames(array $config): array {
    $columns = array_keys($config['fields'] ?? []);
    foreach (['search', 'filters', 'columns'] as $key) {
        foreach (($config[$key] ?? []) as $column) {
            if (is_string($column)) $columns[] = $column;
        }
    }
    foreach (['date_column', 'unique'] as $key) {
        if (!empty($config[$key]) && is_string($config[$key])) $columns[] = $config[$key];
    }

    $virtualColumns = adminModuleVirtualColumns();
    return array_values(array_unique(array_filter($columns, static fn($column) => !in_array($column, $virtualColumns, true))));
}

function adminEnsureConfiguredColumnsForTable(string $table, array $columns): void {
    if (!adminTableExists($table) || !$columns) return;
    $definitions = array_merge(adminCanonicalColumnDefinitions($table), adminModuleOptionalColumns()[$table] ?? []);
    $wanted = [];
    foreach ($columns as $column) {
        if (isset($definitions[$column])) $wanted[$column] = $definitions[$column];
    }
    if ($wanted) ensureTableColumns($table, $wanted);
}

function adminEnsureModuleConfiguredColumns(array $config): void {
    adminEnsureConfiguredColumnsForTable($config['table'], adminModuleConfiguredColumnNames($config));
    $requiredTables = adminModuleRequiredTables($config);
    foreach (adminModuleDefinitions() as $definition) {
        if (!in_array($definition['table'] ?? '', $requiredTables, true)) continue;
        adminEnsureConfiguredColumnsForTable($definition['table'], adminModuleConfiguredColumnNames($definition));
    }
}
...
function adminEnsureModuleTables(array $config): void {
    ...
    adminEnsureModuleConfiguredColumns($config);
}
...
function adminModuleVirtualColumns(): array {
    return ['match_title','category_title','form_title','prediction_count','winner_count','prediction_team_one','prediction_team_two','prediction_score_one','prediction_score_two','final_score_one','final_score_two','crm_status','team_one_display','team_two_display','match_start_display','prediction_start_display','prediction_end_display','points_reward_display','user_phone','user_full_name','order_number','order_total','predicted_scores'];
}

function adminModuleNormalizeConfig(array $config, string $moduleKey = ''): array {
    $table = (string)($config['table'] ?? '');
    $pagePath = $_SERVER['SCRIPT_NAME'] ?? ($_SERVER['PHP_SELF'] ?? 'unknown');
    if ($table === '') {
        safeAdminLog('Admin module normalization failed: module=' . $moduleKey . ' table= page=' . $pagePath . ' error=missing-table');
        throw new RuntimeException('Module table is not configured.');
    }

    if (!adminTableExists($table)) {
        ensureAdminCanonicalTables(adminDb(), [$table]);
    }
    if (!adminTableExists($table)) {
        safeAdminLog('Admin module normalization failed: module=' . $moduleKey . ' table=' . $table . ' page=' . $pagePath . ' error=missing-table-after-repair');
        throw new RuntimeException('Required table is missing: ' . $table);
    }

    adminEnsureModuleOptionalColumns($table);
    adminEnsureConfiguredColumnsForTable($table, adminModuleConfiguredColumnNames($config));

    $allowed = adminModuleExistingColumnNames($table);
    if (!$allowed) {
        safeAdminLog('Admin module normalization failed: module=' . $moduleKey . ' table=' . $table . ' page=' . $pagePath . ' configured=' . implode(',', array_keys($config['fields'] ?? [])) . ' db_columns= error=columns-not-readable');
        throw new RuntimeException('Table exists but columns are not readable: ' . $table);
    }

    $originalFields = $config['fields'] ?? [];
    $filteredFields = array_filter($originalFields, static fn($field) => in_array($field, $allowed, true), ARRAY_FILTER_USE_KEY);
    $removedFields = array_values(array_diff(array_keys($originalFields), array_keys($filteredFields)));
    if (!$filteredFields && $originalFields) {
        safeAdminLog(
            'Admin module normalization removed all fields. module=' . $moduleKey .
            ' table=' . $table .
            ' configured=' . implode(',', array_keys($originalFields)) .
            ' db_columns=' . implode(',', $allowed) .
            ' removed=' . implode(',', $removedFields) .
            ' page=' . $pagePath
        );
        throw new RuntimeException('Configured fields do not match database columns for table: ' . $table);
    }
    if ($removedFields) {
        safeAdminLog(
            'Admin module normalization removed unavailable fields. module=' . $moduleKey .
            ' table=' . $table .
            ' configured=' . implode(',', array_keys($originalFields)) .
            ' db_columns=' . implode(',', $allowed) .
            ' removed=' . implode(',', $removedFields) .
...
function adminRenderModulePage(string $moduleKey): void {
    ...
    try {
        ...
        $config = adminModuleNormalizeConfig($config, $moduleKey);
