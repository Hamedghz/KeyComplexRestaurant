...
function adminRenderModulePage(string $moduleKey): void {
    ...
    try {
        ...
        $repair = adminRepairCriticalModuleSchema($config);
        if (empty($repair['ok']) && !empty($repair['missing_after_repair'])) {
            safeAdminLog('Critical module schema repair incomplete (' . $moduleKey . '): ' . implode(', ', $repair['missing_after_repair']));
        }
        ...
        if (empty($config['fields']) && empty($config['readonly_create'])) {
            safeAdminLog('Admin module has no editable fields after schema normalization (' . $moduleKey . ' -> ' . ($config['table'] ?? 'unknown') . ')');
        }
        if (($_GET['export'] ?? '') === 'csv' && !$error) {
            adminModuleExportCsv($config);
        }
    } catch (Throwable $e) {
        $error = 'آماده‌سازی ساختار داده انجام نشد. لطفاً لاگ سیستم را بررسی کنید.';
    ...
    if (in_array($action, ['add', 'edit'], true) && empty($config['fields'])) {
        if (!$error) {
            $error = 'فرم این صفحه با ستون‌های واقعی جدول «' . ($config['table'] ?? 'نامشخص') . '» همگام نیست. فیلدهای ناموجود: هیچ فیلد قابل ویرایشی پیدا نشد';
        }
        $action = 'list';
    }
    $editRow = null;
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$error) {
        try {
            ...
            if ($crudAction === 'delete') {
                if (empty($config['allow_delete']) && array_key_exists('allow_delete', $config)) {
                    throw new RuntimeException('حذف رکورد برای این صفحه فعال نیست.');
                }
            ...
            if ($crudAction === 'save') {
                ...
                if (($config['table'] ?? '') === 'matches' && function_exists('adminRecalculatePredictionsForMatch')) {
                    adminRecalculatePredictionsForMatch($savedId);
                }
                redirectTo(basename($_SERVER['PHP_SELF']) . '?saved=1');
            }
        } catch (PDOException $e) {
            safeAdminLog('Admin module save failed (' . $moduleKey . '): ' . $e->getMessage());
