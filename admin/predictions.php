...
$config = adminModuleDefinition('predictions');
if (!$config) {
...
try {
    ...
    safeAdminLog('Admin module bootstrap failed (predictions): ' . $e->getMessage());
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$error) {
    try {
        ...
        safeAdminLog('Admin module save failed (predictions): ' . $e->getMessage());
...
try {
    ...
    safeAdminLog('Admin module rows failed (predictions): ' . $e->getMessage());
}
