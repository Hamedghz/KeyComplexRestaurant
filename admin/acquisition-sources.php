...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $id = (int)($_POST['id'] ?? 0);
        if ($action === 'delete') {
            $db->prepare('UPDATE acquisition_sources SET active = 0 WHERE id = ?')->execute([$id]);
            redirectTo('acquisition-sources.php?deactivated=1');
        }
...
?>
<?php if (!empty($_GET['deactivated'])): ?><div class="alert alert-info">منبع جذب غیرفعال شد.</div><?php endif; ?>
